...
def create_train_data(all_classes, all_labels):
    ...
    for label_index, specific_class in enumerate(all_classes):
        ...
        for img_filename in os.listdir(current_dir):
            ...
            img = cv2.imread(path)
            if img is None:
                print(f"    ❌ Could not read {img_filename}, skipping")
                continue
...
split = int(len(training_data) * (PERCENT_TRAINING_DATA / 100))
...
# Train the model
model.fit(X_train, Y_train, epochs=NUM_EPOCHS, batch_size=32, validation_data=(X_test, Y_test), verbose=1,callbacks=[early_stop])
...
print(classification_report(y_true, y_pred_classes, target_names=all_classes))

print(confusion_matrix(y_true, y_pred_classes))


# Save the model
model.save(f"../trained_data/{MODEL_NAME}.h5")